...
def test_KeyCode():
    assert peg.parse("<AE01>", xkbparse.KeyCode).name == "AE01"
    assert peg.parse("<MDSW>", xkbparse.KeyCode).name == "MDSW"
...
def test_KeySymLevels():
    with open(os.path.join(TEST_DATA_DIR, "list_AE01")) as f:
        parsed = peg.parse(f.read(), xkbparse.KeySymLevels)
...
def test_KeyDefSymbols():
    with open(os.path.join(TEST_DATA_DIR, "symbols_AE01")) as f:
        parsed = peg.parse(f.read(), xkbparse.KeyDefSymbolsGroup)
        assert parsed.num == 1
...
def test_KeyDef():
    with open(os.path.join(TEST_DATA_DIR, "AE01")) as f:
        parsed = peg.parse(f.read(), xkbparse.KeyDef)
        ...
        composed = peg.compose(parsed, xkbparse.KeyDef)
        # print(composed)
        reparsed = peg.parse(composed, xkbparse.KeyDef)
